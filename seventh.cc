...
main(int argc, char* argv[])
{
    ...
    if (!useV6)
    {
        ...
        sourceAddress = InetSocketAddress(interfaces.GetAddress(0), sinkPort);
    ...
    //TODO: GO FROM SINK HELPER TO HAVING ANOTHER SOCKET
    //SO WE CAN DYNAMICALLY SET ATTRIBUTES
    /*Ptr<Socket> socketRecv = Socket::CreateSocket(nodes.Get(1), TcpSocketFactory::GetTypeId());
    Ptr<RecvApp> receiver = CreateObject<RecvApp>();
    receiver->Setup(socketRecv, sourceAddress, sinkAddress);
    nodes.Get(1)->AddApplication(receiver);
    receiver->SetStartTime(Seconds(0.));
    receiver->SetStopTime(Seconds(20.));*/
    //TODO: MAKE APPLICATIOON FOR RECEPTION
    ...
    //Ptr<Socket> recvSock = packetSinkHelper.GetListeningSocket();
    //recvSock->SetAttribute(DelAckCount", UintegerValue(5));
    ...
    //sinkApps.SetAttribute("DelAckCount", UintegerValue(5));
    ...
    pointToPoint.EnablePcapAll("full_pcap_7th");
    Config::Set("/NodeList/1/$ns3::TcpSocket/DelAckCount", UintegerValue(5));
    Config::Set("/NodeList/0/$ns3::TcpSocket/DelAckCount", UintegerValue(5));
    Simulator::Run();
    Simulator::Destroy();
